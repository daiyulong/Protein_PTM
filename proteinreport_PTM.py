...
Add: 上海市闵行区紫月路468号5楼 | Web: www.bioprofile.cn | Tel: 4008208531 | Email: info@bioprofile.cn</div></div></body></html>""")
    ...
    def print_all_protein_function_analysis(self):
        ...
        if os.path.exists(self.pn+'/3.AllProtein'):
    ...
    def print_dep_protein_function_analysis(self):
        ...
        <p>结果目录：<span class="text-primary">5.DEPStatistics\\*.vs.*</span>，其中红色表示显著上调差异修饰位点，蓝色表示显著下调差异修饰位点。
        统计结果请见表{0}-{1}。</div></p>
      ...
      <th scope="col">显著差异位点总数</th></tr>
        ...
        <p>下图展示了差异修饰位点在两组样本中的聚类热图，颜色越红表示相对表达量越高，颜色越蓝表示相对表达量越低。</p>
        <p>聚类热图结果目录：<span class="text-primary">5. DEPStatistics\\*.vs.*\\*.vs.*.accession.pheatmap.png</span></p>
        </div>
        """)
        ...
        for comp in self.comp_group:
            ...
            content = content + self.return_image(depstaturl + '/' + comp + '/' + comp + '.accession.pheatmap.png',
                                                           "图{}-{} {}组的热图".format(self.index, figid, comp))
        ...
        #差异修饰位点丰度分析
        #dpo.write("<h2>{}.{} 差异修饰位点丰度分析</h2>".format(self.index, h2id))
        self.print_h2(dpo, "差异修饰位点丰度分析", h2id, "dep.html",
                      "dep{}".format(h2id))
        h2id = h2id + 1
        dpo.write("""<div><p>下图展示了某个差异修饰位点在两组中的平均表达情况，横坐标为两个组别，纵坐标为相对表达量。结果提供了柱状图、箱线图
        、抖图和散点图。其中BarComp图中显示的p-value是采用Welch T检验，图中标有具体数值。同时用*表示p&lt;0.05，**表示p&lt;0.01，
        ***表示p&lt;0.001，空白表示p&gt;0.05（即没有显著性）。</p>
        ...
        self.print_image(dpo, depstaturl + '/' + self.comp_group[0] + '/'+ self.comp_group[0] +'.bar/1.png',
                         "图{}-{} 差异修饰位点两组表达柱状图".format(self.index, figid),400)
        figid = figid + 1

        self.print_image(dpo, depstaturl + '/' + self.comp_group[0] + '/' + self.comp_group[0] + '.box/1.png',
                         "图{}-{} 差异修饰位点两组表达箱线图".format(self.index, figid), 400)
        figid = figid + 1

        self.print_image(dpo, depstaturl + '/' + self.comp_group[0] + '/' + self.comp_group[0] + '.jitter/1.png',
                         "图{}-{} 差异修饰位点两组表达抖图".format(self.index, figid), 400)
        figid = figid + 1

        self.print_image(dpo, depstaturl + '/' + self.comp_group[0] + '/' + self.comp_group[0] + '.scatter/1.png',
                         "图{}-{} 差异修饰位点两组表达散点图".format(self.index, figid), 400)
        figid = figid + 1

        # 差异修饰位点motif分析
        self.print_h2(dpo, "差异修饰位点motif分析", h2id, "dep.html",
                      "dep{}".format(h2id))
        h2id = h2id + 1
        dpo.write("""<div><p>下图展示了差异修饰位点motif分析情况，分别对全部的差异修饰位点，差异上调和差异下调的修饰位点均进行了分析。
        </p><p>横坐标表示氨基酸在Sequence window(以S-丝氨酸/T-苏氨酸/Y-酪氨酸作为中心上下游各6个氨基酸序列)中的位置，纵坐标表示氨基
        酸出现在每个位点的相对频率。</p><p>结果目录：<span class="text-primary">7.DEPMotif</span></p></div>""")

        dpo.write("""<div class="text-center"><img src="img/motif.png" width=800 /></div>""")
        # self.print_image(dpo, depstaturl + '/' + self.comp_group[0] + '/' + self.comp_group[0] + '.bar/1.png',
        #                  "图{}-{} 差异修饰位点两组表达柱状图".format(self.index, figid), 400)
        figid = figid + 1
        ...
        注释结果，横坐标为level 2的GO term，左侧纵坐标为count（注释到该GO term的差异蛋白数量），右侧纵坐标为percentage（注释到该GO term的差
        异蛋白数量 / 有GO注释的差异蛋白总数量），3大分支：生物过程（Biological Process，BP）、分子功能（Molecular
         Function，MF）和细胞组分（Cellular Component，CC）分别用不同的颜色表示。每个柱子上方标有具体的count值。</p>
         <p>结果目录：<span class="text-primary">6. DEPFunction\*.vs.*\*.vs.*.all_GO_Level2\GOlevel2CountBar</span></p></div>
        """)
        ...
        # DAG图
        dpo.write("""<div><br /><br /><p>下图展示了差异蛋白富集GO term的有向无循环图。对显著性最高的前10个GO term作为有向无循环图的主节点，
        ...
        self.print_h2(dpo, "差异位点的蛋白KEGG注释与富集", h2id, "dep.html",
                      "dep{}".format(h2id))
        h2id = h2id + 1
        ...
        性状或疾病的发生机理、药物作用机制等最直接和必要的途径，将有助于我们去了解蛋白的功能和蛋白间的关系。</p><p>KEGG是常用于通路研究的数据
        库之一，对目标蛋白集合进行基于KEGG数据库通路注释，具体的通路注释列表请见附件中的对应KEGG文件夹，并可以通过列表（KEGG.enriched.xlsx）
        中的KEGG通路中的URL链接打开并下载相关KEGG通路注释图谱。图中的方框为蛋白质/基因，圆点为代谢物，其中蛋白质/基因用红色填充标识。</p>
        <p>结果目录：6. DEPFunction\\*.vs.*\\*.vs.*.all_KEGG\\KEGG.enriched.xlsx，可以通过列表中的URL查看并下载相关通路注释图谱。
        </p></div>""")
        dpo.write("""<div><p>依据Fisher精确检验的算法对比较组差异蛋白进行功能富集分析。结果用p-value表示，小于0.05为功能显著富集，
        值越小表示功能富集越显著。</p></div>""")

        # KEGG 气泡图
        dpo.write("""<div><br /><br /><p>下图展示了富集显著性top 10的pathway，用气泡图展示，可以将p-value、Rich factor和count
        共计3个参数同时体现在一张图中。横坐标表示rich factor，纵坐标表示p-value的负对数转化，圆圈大小表示count，不同的颜色表示各个pathway。
        </p><p>结果目录：<span class="text-primary">
                6. DEPFunction\\*.vs.*\\*.vs.*.all_KEGG\\Top10EnrichSymbol</span></p> </div>""")
        ...
        表示rich factor，圆圈大小表示count。
                        ...
                        6. DEPFunction\\*.vs.*.\\*.vs.*.all_KEGG\\Top20EnrichedSymbol2</span></p>
                                </div>""")
        ...
        # KEGG 条形图top class
        dpo.write("""<div><br /><br /><p>下图展示了差异位点对应蛋白通路富集显著性top30的KEGG pathway。横坐标为p-value的负对数转化，
        纵坐标为具体通路。柱子颜色归属KEGG level 1分类。柱子右侧有具体的count和p-value。
                        </p><p>结果目录：<span class="text-primary">
                                6. DEPFunction\\*.vs.*\\*.vs.*.all_KEGG\\KEGG.Sig.Bar</span></p></div>""")
        ...
        for comp in self.comp_group:
            ...
            self.print_accordion_element(dpo, "keggsigbar", "keggsigbar" + str(c),
                                         ...
                                         self.return_image(
                                             ...
                                             "图{}-{} {}组的差异蛋白的KEGG pathway富集图（level 1）".format(
                                                 self.index,
                                                 figid, comp)),
                                         show
                                         )
        ...
        dpo.write("""<div><br /><br /><p>下图也展示了差异蛋白通路富集显著性top30的KEGG pathway。横坐标为p-value的负对数转化，
        纵坐标为具体通路。柱子颜色归属KEGG level 2分类。
        </p><p>结果目录：<span class="text-primary">
                                6. DEPFunction\\*.vs.*\\*.vs.*.all_KEGG\\KEGG.Sig.Bar2</span></p> </div>""")
        ...
        for comp in self.comp_group:
            ...
            self.print_accordion_element(dpo, "keggsig2bar", "keggsig2bar" + str(c),
                                         ...
                                         self.return_image(
                                             ...
                                             """图{}-{} {}组的差异蛋白的KEGG pathway富集图（level 2）""".format(
                                                 self.index,
                                                 figid, comp)),
                                         show
                                         )
        ...
        # KEGG top class气泡图
        dpo.write("""<div><br /><br /><p>下图也展示了差异蛋白通路富集显著性top 30的KEGG pathway，用气泡图展示。将KEGG pathway归属到level 1
        分类，在每个类中的KEGG pathway从上至下-log10（p-value）依次降低，即p-value依次升高，显著性依次降低。圆圈大小表示count，颜色表示rich factor。
                </p><p>结果目录：<span class="text-primary">
                                        6. DEPFunction\\*.vs.*\\*.vs.*.all_KEGG\\KEGG.bubble</span></p> </div>""")
        ...
        self.print_h2(dpo, "差异位点的蛋白亚细胞定位分析", h2id, "dep.html",
                      "dep{}".format(h2id))
        h2id = h2id + 1
        ...
        通过分析GO数据库的细胞组分（Cellular Component，CC），对差异蛋白的亚细胞定位进行注释和统计，下图展示了亚细胞定位的差异位点对应的蛋白数量
        统计结果，依次为柱形图、气泡图、饼图和环图。下图以全部差异位点对应的蛋白的亚细胞定位分析结果为例</p>
        ...
        self.print_h2(dpo, "差异位点的蛋白功能互作网络（PFIN）分析", h2id, "dep.html",
                      "dep{}".format(h2id))
        h2id = h2id + 1
        ...
        <p>结果目录：<span class="text-primary">6.DEPFunction\\*.vs.*\\*.vs.*.all_PFIN</p>
        </div>""")
        dpo.write("<div>结果链接表：")
        dpo.write("""<table class="table">
            ...
            """)
        for comp in self.comp_group:
            if os.path.exists(depfun + '/' + comp + '/' + comp + ".all_PFIN/pathway-gene_all/web_session/index.html"):
                dpo.write("""
          <tr>
            <td>{0}</td>
          ...
          """.format(comp,depfuncurl + '/' + comp + '/' + comp + ".all_PFIN/pathway-gene_all/web_session/index.html"))
            if os.path.exists(depfun + '/' + comp + '/' + comp + ".all_PFIN/pathway-gene_part/web_session/index.html"):
                dpo.write("""
                <tr>
                  <td>{0}</td>
                ...
                """.format(comp, depfuncurl + '/' + comp + '/' + comp + ".all_PFIN/pathway-gene_part/web_session/index.html"))
            if os.path.exists(depfun + '/' + comp + '/' + comp + ".all_PFIN/pathway-gene-ppi_all/web_session/index.html"):
                dpo.write("""
                <tr>
                  <td>{0}</td>
                ...
                """.format(comp, depfuncurl + '/' + comp + '/' + comp + ".all_PFIN/pathway-gene-ppi_all/web_session/index.html"))
            if os.path.exists(depfun +  '/' + comp + '/' + comp + ".all_PFIN/pathway-gene-ppi_part/web_session/index.html"):
                dpo.write("""
                <tr>
                  <td>{0}</td>
                ...
                """.format(comp, depfuncurl + '/' + comp + '/' + comp + ".all_PFIN/pathway-gene-ppi_part/web_session/index.html"))
            if os.path.exists(depfun + '/' + comp + '/' + comp + ".all_PFIN/pathway-pathway/web_session/index.html"):
                dpo.write("""
                   <tr>
                     <td>{0}</td>
                   ...
                   """.format(comp,
                              depfuncurl + '/' + comp + '/' + comp + ".all_PFIN/pathway-pathway/web_session/index.html"))
            if os.path.exists(depfun + '/' + comp + '/' + comp + ".all_PFIN/ppi/web_session/index.html"):
                dpo.write("""
                              <tr>
                                <td>{0}</td>
                                <td>蛋白相互作用网络</td>
                              ...
                              """.format(comp,
                                         depfuncurl + '/' + comp + '/' + comp + ".all_PFIN/ppi/web_session/index.html"))
    ...
    def print_sample_compare_analysis(self):
        ...
        scfold = self.pn + '/2.SampleAnalysis'
        scurl = '../2.SampleAnalysis'
        figid = 1
        ...
        if os.path.exists(scfold):
            ...
            sco.write("""<div><p>根据修饰位点在不同样本间的表达情况，对样本进行表达分布、相关性分析和PCA分析。根据结果可以从各个角度评估
            组间差异性和组内重复性。</p><p>以下箱线图和小提琴图中的同组样本中位数接近于同一水平线，直方图接近于钟形分布，说明数据质量越好，重
            复性良好。</p><p>结果目录：<a href="{}" target=_blank>2.SampleAnalysis</a></p>
            </div>""".format(scurl))
            ...
            if os.path.exists(scfold+'/corr.png'):
                sco.write("""以下为样本相关性图。相关性系数由-1至1，＜0为负相关，＞0为正相关，绝对值越大表示相关性越强。即同组样本间的相关
                性系数越接近于1越好。""")
    ...
    def print_multi_group_comparison_analysis(self, index):
        self.index = index
        anovafold = self.pn+'/8.ANOVA'
        anovaurl = '../3.AllProtein/3.1Multi_group_expression/kmeans'
        if os.path.exists(anovafold):
            ...
            mco.write("""<div><p>分析修饰位点在多组中的表达量变化趋势，采用单因素方差分析（ANOVA）检验方法。<p></div>""")
            ...
            mco.write("""<div><p>对显著性p-value&lt;0.05的修饰位点进行可视化。
            横坐标为不同组，纵坐标为相对表达量。其中BarComp的两组间显著性采用T检验，用*表示p&lt;0.05，**表示p&lt;0.01，
            ***表示p&lt;0.001，空白表示p&gt;0.05（即没有显著性）。</p>
            <p>注：仅展示显著性top300的蛋白</p><p>
            <p>ANOVA检验结果：<span class="text-primary">
            8.ANOVA\\[group name]\\kmeans\\ANOVA_result.xlsx</span></p></div>
            """)
            ...
            <p>下图是全部样本的k-means聚类图，绿色线代表该类中每一个修饰位点的表达趋势，红色线表示该类的均值。</p></div>""")
            anovaurl="../8.ANOVA"
            ...
            self.print_h2(mco, "GO功能和KEGG通路分析", h2id, 'anova.html',
                          "multicomp{}".format(h2id))
            h2id = h2id + 1
            mco.write("<div><p>ANOVA检验得到的p&lt;0.05的修饰位点对应蛋白的功能分析结果的说明可以参考差异修饰位点对应蛋白的功能分析结果:</p></div>")
            mco.write("""<table class="table"><thead><tr><td>组别</td><td>类型</td><td>目录</td></tr></thead>
            <tbody>""")
            ...
            for at in anovagroup:
                if os.path.exists(anovafold+ '/'+at):
                    if os.path.exists(anovafold+ '/'+at+'/GOEnrich_Identified'):
                        mco.write("""<tr><td>{0}</td><td>{1}</td><td><a href="{2}" 
                        target="_blank">{3}</a></td></tr>""".format(at, '以全部鉴定到蛋白质为背景参照的GO富集分析结果', '../8.ANOVA/'+at+'/GOEnrich_Identified','8 .ANOVA/'+at+'/GOEnrich_Identified'))
                    if os.path.exists(anovafold + '/' + at + '/GOEnrich_Species'):
                        mco.write("""<tr><td>{0}</td><td>{1}</td><td><a href="{2}" 
                                                target="_blank">{3}</a></td></tr>""".format(at, '以该物种全部基因为背景参照的GO富集分析结果',
                                                                                            '../8.ANOVA/'+at+'/GOEnrich_Species',
                                                                                            '8.ANOVA/'+at+'/GOEnrich_Species'))
                    ...
                    if os.path.exists(anovafold + '/' + at + '/GO_subcellular_localization'):
                        mco.write("""<tr><td>{0}</td><td>{1}</td><td><a href="{2}" 
                                                target="_blank">{3}</a></td></tr>""".format(at,
                                                                                            '蛋白质在经典的亚细胞器定位统计结果',
                                                                                            '../8.ANOVA/' + at + '/GO_subcellular_localization',
                                                                                            '8.ANOVA/' + at + '/GO_subcellular_localization'))
...
可以有效地可视化不同数据集之间的关联并揭示其潜在模式。如下图所示，图中展示了相对表达量、FC、p-value和unique肽段数和等多个元素的信息。在数据
分析过程中，老师可以根据需要选择不同的元素进行complex heatmap的定制。</p></div>
...
<div class="text-center">
      ...
      <figcaption class="figure-caption text-center">图{0}-1. 复杂热图（complex heatmap）图例</figcaption>
		...
		连接圆上任意两点的弧线叫做弦，表示两点之间存在关联关系，这种形式非常适合分析复杂数据之间的关联关系。另外，热图也可以通过circos圈图来
		展示，相比常规热图，在展示较多的features时圈图的可视化效果更好。</p></div>
                <div class="text-center">
                <figure class="figure">
		...
		的总和与所有分支宽度的总和相等，保持能量平衡，适用于可视化分析上下调蛋白与功能或者修饰位点（修饰组学）与蛋白和功能之间的数据流走向分析。</p></div>
                <div class="text-center">
                <figure class="figure">
...
""".format(self.index, self.index + 1, self.index + 2, self.index + 3))
    ...
    def print_lfq_english(self):
        ...
        ero.write("""
        	...
        	<h2>Materials and Methods</h2>
        	<h2>Reagents</h2>
        	<p>Ammonium bicarbonate, dithiothreitol (DTT), iodoacetamide (IAA), and sodium carbonate, formic acid (FA) 
        	were purchased from Sigma-Aldrich (St. Louis, MO). Urea and Sodium dodecyl sulfate (SDS) were purchased 
        	from Bio-Rad (Hercules, CA). Acetonitrile (ACN) and water for nano-LC−MS/MS were purchased from J. T. Baker
        	 (Phillipsburg, NJ). Trypsin was purchased from Promega (Madison, WI). All other chemical reagents were 
        	 purchased with analytical grade.</p>
        	 
        	<h2>Sample preparation and protein digestion</h2>
        	<p>Tissue samples were ground into powder using liquid nitrogen. Approximately 20 mg of powder were resuspended 
        	in 200 μL lysis buffer (4% SDS, 100 mM DTT, 150 mM Tris-HCl pH 8.0). The samples were boiled and further 
        	ultrasonicated. Undissolved cellular debris were removed by centrifugation at 16,000 x g for 15 min. The 
        	supernatant was collected and quantified with a BCA Protein Assay Kit (Bio-Rad, USA). Digestion of protein 
        	was performed according to the FASP procedure described by Wisniewski, Zougman et al [1]. The proteins were 
        	transferred into 1.5-mL Nanosep tube (Pall Corporation,MWCO 10K) and centrifuge for 20 min at 14,000 x g at 
        	20 ºC to remove the DTT. Then 100 μL 0.05 M IAA in UA buffer was added to block reduced cysteine residues and 
        	the samples were incubated for 20 min in darkness. Then, add 200 µL buffer Ammonium bicarbonate (100mM) to 
        	the beads in the filter cartridge, centrifuge for 15 min (or until the 200 µL buffer pass through the filter) 
        	at 14,000 x g at 25 ºC. Finally, added 50-100 μL of 50mM Ammonium bicarbonate buffer with trypsin at the enzyme-
        	to-protein ratio of 1:50 into the sample and incubated at 37℃ for 20h. The peptides were harvested by centrifugation,
        	 then acidified by 1% FA and subsequently dried by a refrigerated CentriVap concentrator (Labconco, Kansas, MO). 
        	 Finally, the tryptic digested peptides were desalted with C18 stage tip.</p>

        	<h2>Phosphoproteomics sample preparation</h2>
        	<p>For phosphopeptide enrichment, High-Select™ TiO2 Phosphopeptide Enrichment kit from Thermo Scientific were 
        	used for digested peptide mixtures from each sample. Briefly, The TiO2 Spin Tip were equilibrated with Wash 
        	Buffer and Binding/Equilibration Buffer by concentration, sequentially. The desalted peptides were suspended 
        	in 150 μL binding/equilibration buffer and applied to the TiO2 spin tip. Afterwards, the TiO2 spin tips were 
        	washed twice with   Binding/Equilibration buffer and Washing buffer sequentially. Then LC-MS grade water were 
        	used for washing once and the phosphopeptides were eluted from TiO2 spin tip by using 50 μL Elution Buffer 
        	twice. The elutes of phosphopeptides were dried immediately and suspended with 0.1% FA for further LC-MS/MS 
        	analysis. </p>
        	
        	<h2>LC-MS Analysis</h2>
        	<p>The phosphopeptides were loaded onto a the C18-reversed phase column (15cm long, 75μm ID, 2μm, Dr. Maisch 
        	GmbH, Ammerbuch, Germany)) in buffer A (2% ACN and 0.1% FA) and separated with a linear gradient of buffer B 
        	(90% ACN and 0.1% FA) at a flow rate of 300 nL/min over 60 min. The linear gradient was set as follows: 0–2 
        	min, linear gradient from 2% to 5% buffer B; 2–42 min, linear gradient from 5% to 20% buffer B; 42–50 min, 
        	linear gradient from 20% to 35% buffer B; 50–52 min, linear gradient from 35% to 90% buffer B; 52–60 min, 
        	buffer B maintained at 90%.LC-MS/MS were performed on a Q Exactive HFX mass spectrometer coupled with Easy 
        	1200 nLC (Thermo Fisher Scientific). Peptide was first loaded to a trap column (100μm*20mm, 5μm，C18，Dr.Maisch 
        	GmbH, Ammerbuch, Germany) in buffer A (0.1% Formic acid in water). Reverse-phase high-performance liquid 
        	chromatography (RP-HPLC) separation was performed with the EASY-nLC system (Thermo Fisher Scientific, Bremen, 
        	Germany) using a self-packed column (75 μm × 150 mm; 3 μm ReproSil-Pur C18 beads, 120 Å, Dr. Maisch GmbH, 
        	Ammerbuch, Germany) at a flow rate of 300 nL/min. The RP−HPLC mobile phase A was 0.1% formic acid in water, 
        	and B was 0.1% formic acid in 95% acetonitrile. Peptide were eluted over 120 min with a linear gradient of 
        	buffer B. MS data was acquired using a data-dependent top20 method dynamically choosing the most abundant 
        	precursor ions from the survey scan (300–1800 m/z) for HCD fragmentation. The instrument was run with peptide 
        	recognition mode enabled. A lock mass of 445.120025 Da was used as internal standard for mass calibration. 
        	The full MS scans were acquired at a resolution of 70,000 at m/z 200, and 17,500 at m/z 200 for MS/MS scan. 
        	The maximum injection time was set to for 50 ms for MS and 50 ms for MS/MS. Normalized collision energy was 
        	27 and the isolation window was set to 1.6 Th. Dynamic exclusion duration was 30 s.</p>	
        	
        	<h2>Sequence Database Searching and Data Analysis</h2>
        	<p>The MS data were analyzed for data interpretation and protein identification against the [物种名称] database 
        	from Uniprot (downloaded on 01/02/2021, and including 88069 protein sequences), which is sourced from the 
        	protein database at https://www.uniprot.org/uniprot/?query=taxonomy:10090. （根据我们的报告数据库情况进行修改）The 
        	MS spectra were searched with Spectronuant version 16. The FDR cutoff was set to 1%. The precursor peptide 
        	and q value cutoffs were 0.2 and 0.01, respectively. The protein q value experiment and run wide cutoffs 
        	were 0.01 and 0.05, respectively. The dataset was analyzed with a sparse q value, and no imputation was 
        	performed. For phosphoproteomics experiments, the PTM localization cutoff was set to 0. The results were 
        	filtered for the best N fragments per peptide between 3 and 25.</p>	
            
            <h2>Bioinformatics analysis</h2>
            <p>Analyses of bioinformatics data were carried out with Perseus software [10], Microsoft Excel, Python and R 
            statistical computing software. K-Nearest Neighbors (KNN) algorithm was used for imputing missing values and 
            resampling datasets. Differentially significant expressed sites were screened with the cutoff of a ratio 
            fold-change of >2 or <0.5 and P-values < 0.05. Expression data were grouped together by hierarchical clustering 
            according to site level, which is based on the open-source statistical language R25, using Euclidean distance 
            as the distance metric and complete method as the agglomeration method. To annotate the sequences, information 
            was extracted from UniProtKB/Swiss-Prot [11], Kyoto Encyclopedia of Genes and Genomes (KEGG) [12], and Gene 
            Ontology (GO) [13]. GO and KEGG enrichment analyses were carried out with the Fisher’s exact test, and FDR 
            correction for multiple testing was also performed. GO terms were grouped into three categories: biological 
            process (BP), molecular function (MF), and cellular component (CC) [13]. Enriched GO and Kegg pathways were 
            nominally statistically significant at the p<0.05 level. Construction of protein–protein interaction (PPI) 
            networks were also conducted by using the STRING database with the Cytoscape software [14]. The statistically 
            significant enriched phosphorylation motifs of kinase-substrates were discovered by the MOMO software [15].</p> 
...
<h6>[15] Timothy L. Bailey, James Johnson, Charles E. Grant, William S. Noble, "The MEME Suite", Nucleic Acids 
Research, 43(W1):W39-W49, 2015.</h6>
        	<h1>{3}. 附录<a name="lit4"></a></h1>
        		...
        		<h4>|-- 2.SampleAnalysis/  样本比较分析目录</h4>
        		 ...
        		 <h5>|&nbsp;&nbsp;|--Correlation_bubble.pdf   样本相关性分析气泡图</h5>
        		 <h5>|&nbsp;&nbsp;|--Correlation_bcatter.pdf   样本相关性分析散点图</h5>
        		 <h5>|&nbsp;&nbsp;|--PCA1.2.pdf, PCA1.2_style2.pdf, PCA1.2_style3   不同风格的二维PCA分析得分图</h5>
                 <h5>|&nbsp;&nbsp;|--PCA123.pdf 三维PCA分析得分图</h5>
        		<h4>|-- 3.AllProtein/  全蛋白表达与功能注释结果目录</h4>
        		  <h6>|&nbsp;&nbsp;|--summary.xlsx  全蛋白注释结果汇总表</h6>
        		 <h5>|&nbsp;&nbsp;|--3.1GO/  GO term注释与富集分析结果目录（模式生物）</h5>
        		  <h6>|&nbsp;&nbsp;|&nbsp;&nbsp;|--GOlevel2CountBar.pdf   GO term注释统计柱状图（level2)</h6>
        		  <h6>|&nbsp;&nbsp;|&nbsp;&nbsp;|--pCountPoint.pdf  GO term富集气泡图（3大分支，横坐标p-value）</h6>
        		  <h6>|&nbsp;&nbsp;|&nbsp;&nbsp;|--pRFPoint.pdf  GO term富集气泡图（3大分支，横坐标RF）</h6>
        		  <h6>|&nbsp;&nbsp;|&nbsp;&nbsp;|--MF/BP/CC.EnrichedBar.pdf  GO term富集柱状图（单独分支）</h6>
        		  ...
        		  <h6>|&nbsp;&nbsp;|&nbsp;&nbsp;|--MF/BP/CC.EnrichedSymbol.pdf  GO term top20富集气泡图（单独分支）</h6>
        		  <h6>|&nbsp;&nbsp;|&nbsp;&nbsp;|--MF/BP/CC.DAG.pdf  GO term富集有向无循环图（单独分支）</h6>
        		 <h5>|&nbsp;&nbsp;|--3.2KEGG/  KEGG通路注释与富集分析结果目录（模式生物）</h5>
        		  <h6>|&nbsp;&nbsp;|&nbsp;&nbsp;|--Top10EnrichedBar.pdf     KEGG通路富集柱状图（top10）</h6>
        		  <h6>|&nbsp;&nbsp;|&nbsp;&nbsp;|--Top10EnrichSymbol.pdf     Top10 KEGG通路富集气泡图（top10）</h6>
        		  <h6>|&nbsp;&nbsp;|&nbsp;&nbsp;|--Top20EnrichedSymbol2.pdf       Top20 KEGG通路富集气泡图（top20）</h6>
        		  <h6>|&nbsp;&nbsp;|&nbsp;&nbsp;|--KEGG.Sig.Bar.pdf  Top30 KEGG通路富集柱状图（颜色归属level1）</h6>
        		  <h6>|&nbsp;&nbsp;|&nbsp;&nbsp;|--KEGG.Sig.Bar2.pdf      Top30 KEGG通路富集柱状图（颜色归属level2）</h6>
        		  <h6>|&nbsp;&nbsp;|&nbsp;&nbsp;|--kegg.sig.pdf  KEGG通路富集top20气泡图（显示level 1分类）</h6>
        		 <h5>|&nbsp;&nbsp;|--3.3subcellular_localization/  亚细胞定位（CC）分析结果目录</h5>
        		  <h6>|&nbsp;&nbsp;|&nbsp;&nbsp;|--CellLocationMerge.png    亚细胞定位统计结果（柱状图、点图、饼图、环图）</h6>
        		<h4>|-- 4.MotifAnalysis/  全蛋白Motif分析结果目录</h4>
        		 <h5>|&nbsp;&nbsp;|--momo.html    网页版Motif分析结果</h5>
                 <h5>|&nbsp;&nbsp;|--*.png   Motif分析结果图</h5>
        		<h4>|-- 5.DEPStatistics/  表达差异分析结果目录</h4>
                 <h5>|&nbsp;&nbsp;|--Venn/   Venn分析结果（至少2组比较)</h5>
        		 <h5>|&nbsp;&nbsp;|--Venn.pdf          各组蛋白鉴定数量韦恩图</h5>
        		 <h5>|&nbsp;&nbsp;|--Venn.xlsx          韦恩分析统计表（表头说明见下表6）</h5>
                 <h5>|&nbsp;&nbsp;|--*.vs.*  各比对组结果</h5>
        		 <h6>|&nbsp;&nbsp;|&nbsp;&nbsp;|--*.vs.*.volcano.pdf  各比对组差异修饰位点可视化火山图</h6>
        		 <h6>|&nbsp;&nbsp;|&nbsp;&nbsp;|--*.vs.*.FoldChange.scatter.pdf  各比对组差异修饰位点FC可视化散点图</h6>
        		 <h6>|&nbsp;&nbsp;|&nbsp;&nbsp;|--*.vs.*.pvalue.scatter.pdf  各比对组差异修饰位点p-value可视化散点图</h6>
        		 <h6>|&nbsp;&nbsp;|&nbsp;&nbsp;|--*.vs.*.updown.bar.pdf  各比对组上下调差异修饰位点统计柱状图</h6>
        		 <h6>|&nbsp;&nbsp;|&nbsp;&nbsp;|--*.vs.*.DEP.Donut.pdf  各比对组差异修饰位点统计环图</h6>
        		 <h6>|&nbsp;&nbsp;|&nbsp;&nbsp;|--*.vs.*.heatmap.pdf  各比对组差异修饰位点聚类热图（两组）</h6>
        		 <h6>|&nbsp;&nbsp;|&nbsp;&nbsp;|--*.vs.*.heatmap-Multi.pdf  各比对组差异修饰位点聚类热图（至少三组）</h6>
        		 <h6>|&nbsp;&nbsp;|&nbsp;&nbsp;|--*.vs.*.heatmap.xlsx  各比对组差异修饰位点聚类统计表</h6>
        		 <h6>|&nbsp;&nbsp;|&nbsp;&nbsp;|--*.vs.*.K-value trend.pdf  各比对组差异修饰位点k值聚类趋势图（至少三组）</h6>
        		 <h6>|&nbsp;&nbsp;|&nbsp;&nbsp;|--*.vs.*.K-value.xlsx  各比对组差异修饰位点k值聚类统计表</h6>
        		 <h6>|&nbsp;&nbsp;|&nbsp;&nbsp;|--*.vs.*.BarComp/BarSE/Boxplot/Jitter/ScatterSE/    各比对组差异修饰位点表达分析结果目录：柱状图/箱线图/抖图/散点图（两组</h6>
        		<h4>|-- 6.DEPFunction/  差异修饰位点蛋白功能分析结果目录</h4>
                 <h5>|&nbsp;&nbsp;|--*.vs.*      比较组的结果</h5>
        		 <h6>|&nbsp;&nbsp;|&nbsp;&nbsp;|--*.vs.*.[all/up/down]_summary.xlsx      all/up/down差异修饰位点蛋白注释汇总表</h6>
                 <h6>|&nbsp;&nbsp;|&nbsp;&nbsp;|--*.vs.*.[all/up/down]_GO_Species/  以该物种全部基因为背景参照的差异修饰位点蛋白GO分析结果目录</h6>
                 <h6>|&nbsp;&nbsp;|&nbsp;&nbsp;|--*.vs.*.[all/up/down]_GO_Level2/  第二层级GO功能的统计结果</h6>
                 <h6>|&nbsp;&nbsp;|&nbsp;&nbsp;|--*.vs.*.[all/up/down]_subcellular_localization/  经典的亚细胞定位的统计结果</h6>
                 <h6>|&nbsp;&nbsp;|&nbsp;&nbsp;|--*.vs.*.[all/up/down]_KEGG  差异修饰位点蛋白KEGG分析结果目录</h6>
        		 <h6>|&nbsp;&nbsp;|&nbsp;&nbsp;|--*.vs.*.all/up/down_PFIN/  all/up/down差异修饰位点蛋白功能互作（PFIN）分析目录</h6>
        		<h4>|-- 7.DEPMotif/  差异修饰位点蛋白Motif分析结果目录</h4>
                 <h5>|&nbsp;&nbsp;|--*.vs.*      比较组的结果</h5>
                 <h6>|&nbsp;&nbsp;|&nbsp;&nbsp;|--*.vs.*.[All/Up/Down]/momo.html  网页版Motif分析结果</h6>
                 <h6>|&nbsp;&nbsp;|&nbsp;&nbsp;|--*.vs.*.[all/up/down]/*.png  Motif分析结果图</h6>
        	<h4>|-- 8.Anova/  多组比较分析结果（如果存在多组比较分析）</h4>
		  <h5>|&nbsp;&nbsp;|--[比较组的名称,由组别加'_'组成<h5>
		   <h6>|&nbsp;&nbsp;|&nbsp;&nbsp;|--Multi_group_expression         多组表达单个修饰位点绘图</h6>
		   <h6>|&nbsp;&nbsp;|&nbsp;&nbsp;|--kmeans         k-means分析结果</h6>
		   <h6>|&nbsp;&nbsp;|&nbsp;&nbsp;|--GOEnrich_Species         以该物种全部基因为背景参照的GO富集分析结果</h6>
		   <h6>|&nbsp;&nbsp;|&nbsp;&nbsp;|--GOLevel2         第二层级GO功能的统计结果</h6>
		   <h6>|&nbsp;&nbsp;|&nbsp;&nbsp;|--GO_subcellular_localization         蛋白质在经典的亚细胞定位的统计结果</h6>
		   <h6>|&nbsp;&nbsp;|&nbsp;&nbsp;|--KEGG         KEGG通路分析结果</h6>
            <div class="gohome">回到顶部</div>
          <script src="js/back.js"></script>
        """.format(self.index, self.index + 1, self.index + 2, self.index + 3))
    ...
    def print_motif_html(self):
        tp2 = open(self.pn + '/html/motif.html', 'w')
        self.print_head_left(tp2)
        tp2url = '../4.MotifAnalysis'

        self.print_h1(tp2, "全可靠修饰位点Motif分析", "motif.html", "motif")
        tp2.write("""<div><p>Motif在生物学中是一个基于数据的数学统计模型，是蛋白激酶识别特定残基的模式，表示激酶对底物的偏好性。
            使用MOMO(http://meme-suite.org/tools/momo)对磷酸化差异位点基序进行分析。</p><p>以下Motif分析图，横坐标表示氨基酸在
            Sequence window(以S-丝氨酸/T-苏氨酸/Y-酪氨酸作为中心上下游各6个氨基酸序列)中的位置，纵坐标表示氨基酸出现在每个位点的相对频
            率。</p><p>结果目录：<a href="{}" target=_blank>4.MotifAnalysis</a></p>
                    </div>""".format(tp2url))
        tp2.write("""<div class="text-center"><img src="img/motif.png" width=800 /></div>""")
        self.print_bottom(tp2)
        tp2.close()
...
if __name__ == '__main__':
    ...
    parser.add_argument('-ia', action='store', dest='pn', default='tmp', help='总的结果目录')
    parser.add_argument('-t', action='store', dest='type', default='DIA', help='报告类型,TMT|LFQ|DIA,default is DIA')
    p = parser.parse_args()
